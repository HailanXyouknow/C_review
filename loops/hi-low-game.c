...
int main(){
  ...
  while(!match && chance_count <= MAX_CHANCES){ // 20 Marks
    ...
    if (user_guess > computer_guess){                     // 10 Marks
      ...
      printf("Remaining chances: %d", MAX_CHANCES - chance_count);
    }
    ...
    // to come out of the loop as the user found the number, do that here.
      ...
      printf("You've guessed it in %d tries", chance_count);
	  // 
    }
  ...
  if (match == 0){                // 10 marks
    printf("Game over!!! Try to be Mathemetical.");                 // 5 marks
  }
  return 0;
}
